  ...
  return {
    read: function (fd) {
      if (arguments[1]) {
        return arguments[1](
          new Error(
            "For performance reasons, skipper-s3 does not support passing in a callback to `.read()`"
          )
        );
      }
    ...
    rm: function (fd, done) {
      _buildS3Client(globalOpts).deleteObjects(
        ...
        (err, result) => {
          ...
          if (result && result["Errors"] && result["Errors"].length > 0) {
            return done(
              flaverr(
                { raw: result["Errors"] },
                new Error("Failed to remove some file(s) from S3 (see `.raw`)")
              )
            );
          }
    ...
    ls: function (dirname, done) {
      _buildS3Client(globalOpts).listObjectsV2(
        _stripKeysWithUndefinedValues({
          Bucket: globalOpts.bucket,
    ...
    receive: function S3Receiver(s3ClientOpts) {
      ...
      receiver._write = (incomingFileStream, encoding, proceed) => {
        ...
        if (
          !_.isString(incomingFileStream.skipperFd) ||
          incomingFileStream.skipperFd === ""
        ) {
          if (
            !_.isString(incomingFileStream.fd) ||
            incomingFileStream.fd === ""
          ) {
            return proceed(
              new Error(
                "In skipper-s3: Incoming file stream does not have the expected `.skipperFd` or `.fd` properties-- at least not as a valid string.  If you are using sails-hook-uploads or skipper directly, this should have been automatically attached!  Here is what we got for `.fd` (legacy property): `" +
                  incomingFileStream.fd +
                  "`.  And here is what we got for `.skipperFd` (new property): `" +
                  incomingFileStream.skipperFd +
                  "`"
              )
            );
        ...
        _uploadFile(
          ...
          (progressInfo) => {
            ...
            if (
              maxBytesPerUpstream &&
              totalBytesWrittenForThisUpstream > maxBytesPerUpstream
            ) {
              wasMaxBytesPerUpstreamQuotaExceeded = true;
              return false;
            } else if (
              maxBytesPerFile &&
              bytesWrittenByFd[incomingFd] > maxBytesPerFile
            ) {
          ...
          (err, { rawS3ResponseData }) => {
            if (err) {
              ...
              if (flaverr.taste({ name: "RequestAbortedError" }, err)) {
                if (
                  maxBytesPerUpstream &&
                  wasMaxBytesPerUpstreamQuotaExceeded
                ) {
                  err = flaverr(
                    { code: "E_EXCEEDS_UPLOAD_LIMIT" },
                    new Error(
                      `Upload too big!  Exceeded quota ("maxBytes": ${maxBytesPerUpstream})`
                    )
                  );
                } else if (maxBytesPerFile && wasMaxBytesPerFileQuotaExceeded) {
                  err = flaverr(
                    { code: "E_EXCEEDS_FILE_SIZE_LIMIT" },
                    new Error(
                      `One of the attempted file uploads was too big!  Exceeded quota ("maxBytesPerFile": ${maxBytesPerFile})`
                    )
                  );
                } //ﬁ
              } //ﬁ
              ...
              incomingFileStream.Location = rawS3ResponseData.Location;
              incomingFileStream.extra = {
                ...rawS3ResponseData,
              };
...
function _uploadFile(
  ...
  const headers = s3ClientOpts.headers || {};
  // https://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/S3.html#upload-property
  var s3ManagedUpload = _buildS3Client(s3ClientOpts).upload(
    _stripKeysWithUndefinedValues({
  ...
  s3ManagedUpload.on("httpUploadProgress", (event) => {
    ...
    let handledSuccessfully = handleProgress(
      _stripKeysWithUndefinedValues({
